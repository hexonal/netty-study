...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    // 出站处理操作， 从 netty 通道 到 nio 通道的操作
    // 端口的绑定，需要通过 nio 通道去完成
    // 最后 是 流水线 的 head 完成， head 里边有个一 unsafe ，可以简单理解为 线程不安全的
    @Override
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        //netty 通道this 注册到  反应器 eventLoop
        @Override
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
            ...
            if (eventLoop.inEventLoop()) {
                ...
                // 执行 底层nio 注册 ， nio channel （封装在 netty channel）要注册到 nio selector  （封装在 netty eventloop）
                register0(promise);
            ...
            try {
                ...
                if (isActive()) {
                    if (firstRegistration) {
                        ...
                        // step3: 服务端传输通道  流水线的激活处理 ，读事件（op_read）的设置
        ...
        public final void bind(final SocketAddress localAddress, final ChannelPromise promise) {
            ...
            //step1 完成 服务端  监听 通道初始化 和 反应器注册

          //  落地到：  nio channel 注册到 selector

            // 注册到 底层selector 上的 selectionkey 的 interesting ops 集合是 0

            // eventloop 查询的时候，查不到 io事件

            //step2: 服务端  监听通道  注册成功之后，进行了监听端口的绑定

            //  落地到：  socket.bind

            //step3: 服务端 流水线的激活处理 ，设置 感兴趣的 读事件，

            //  落地到： selectionkey.insterestOps()
            ...
            if (!wasActive && isActive()) {
                invokeLater(new Runnable() {
                    ...
                    public void run() {
                        //step3: 流水线的激活处理
                        // 开始 nio 事件的查询
                        // selectionkey  设置 感兴趣的io事件，对于服务端 监听通道 需要设置  op_accept

                        // 设置底层的 selectionkey 是一个 出站处理操作， 就有完整的 流水线传播流程， 最终 headcontext 的 read （beginread）
