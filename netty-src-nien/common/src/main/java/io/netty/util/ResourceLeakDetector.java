...
public class ResourceLeakDetector<T> {
    ...
    /** the collection of active resources 活跃资源对象的集合 (应用计数 >0) */
    ...
    private void reportLeak() {
        ...
        if (!logger.isErrorEnabled()) {
            clearRefQueue();  // 仅仅清空当前ReferenceQueue即可
            return;
        }
        // 检查和报告之前所有的泄漏
        for (;;) {
            ...
            if (ref == null) {   // 为空说明已经清空了
                break;
            }
            if (!ref.dispose()) {
                // 如果这个DefaultResourceLeak对象的dispose方法返回false， 活跃资源 集合中已经移除这个 资源
    ...
    private static final class DefaultResourceLeak<T>
        ...
        DefaultResourceLeak(
           ...
           // 调用WeakReference  的 构造方法
            // 注意传入了  ReferenceQueue， 完成GC的通知
            ...
            // 将当前的DefaultResourceLeak 对象 加入到allLeaks （活跃资源对象的集合）集合里面
            // 这个集合是 由它跟踪的资源所属的  ResourceLeakDetector管理
            // allLeaks 集合在后面判断资源是否正确释放扮演重要角色
            allLeaks.add(this);
         ...
         * 那么这个跟踪这个资源的DefaultResourceLeak的Record就会有很多, 但并不是每个记录都需要被记录，否则就会对内存和运行都会造成压力
         * 因为每个Record都会记录整个调用栈
         * 因此需要对记录做取舍,   这个函数 有一个预设的 TARGET_RECORDS字段  长度阈值
         * 这里有几个原则
         * 1. 所有record都会用一根单向链表来保存
         * 2. 最新的record永远都会被记录
         * 3. 小于TARGET_RECORDS  长度阈值 数目的record也会被记录
         * 4. 当数目大于等于TARGET_RECORDS 长度阈值的时候，就会根据概率选择 替换还是  添加到头部。
         * 替换头部，是用最新的record替换掉当前链表中头上的record(保证链表长度不会增加)，添加就是 仅仅添加到头上的record之前
