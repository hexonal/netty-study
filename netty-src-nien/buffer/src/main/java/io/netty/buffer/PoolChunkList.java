...
//        System.out.println("freeMinThreshold =: "+ freeMinThreshold  /1024);
//        System.out.println("freeMaxThreshold =: "+ freeMaxThreshold /1024);
    }
    ...
    boolean free(PoolChunk<T> chunk, long handle, ByteBuffer nioBuffer) {
        chunk.free(handle, nioBuffer);  //释放page 和slib
        //free阈值超过上限，向左移动
        if (chunk.freeBytes > freeMaxThreshold) {
            //从当前的 PoolChunkList中干掉
            remove(chunk);
            // Move the PoolChunk down the PoolChunkList linked-list.
            // 将PoolChunk向下/左移动到PoolChunkList链接列表中。
            return move0(chunk);
        }
        return true;
    }
